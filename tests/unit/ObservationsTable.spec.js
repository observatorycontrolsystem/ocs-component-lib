...
const filterOptions = { 
  ...
  "choice_fields": [
    ...
    { "name": "enclosure", "options": [["clma", "clma"], ["doma", "doma"]] }, 
    { "name": "telescope", "options": [["1m0a", "1m0a"], ["2m0a", "2m0a"]] }, 
    { "name": "state", "options": [["PENDING", "PENDING"], ["IN_PROGRESS", "IN_PROGRESS"], ["NOT_ATTEMPTED", "NOT_ATTEMPTED"], ["COMPLETED", "COMPLETED"], ["CANCELED", "CANCELED"], ["ABORTED", "ABORTED"], ["FAILED", "FAILED"]] }, 
    { "name": "time_span", "options": [["", "---------"], ["today", "Today"], ["yesterday", "Yesterday"], ["week", "Past 7 days"], ["month", "This month"], ["year", "This year"]] }, 
    { "name": "observation_type", "options": [["NORMAL", "NORMAL"], ["RAPID_RESPONSE", "RAPID_RESPONSE"], ["TIME_CRITICAL", "TIME_CRITICAL"], ["DIRECT", "DIRECT"]] }, 
    { "name": "request_state", "options": [["PENDING", "PENDING"], ["COMPLETED", "COMPLETED"], ["WINDOW_EXPIRED", "WINDOW_EXPIRED"], ["FAILURE_LIMIT_REACHED", "FAILURE_LIMIT_REACHED"], ["CANCELED", "CANCELED"]] }, 
    { "name": "instrument_type", "options": [["1M0-SCICAM-SINISTRO", "1M0-SCICAM-SINISTRO"]] }, { "name": "configuration_type", "options": [["AUTO_FOCUS", "AUTO_FOCUS"], ["BIAS", "BIAS"], ["DARK", "DARK"], ["EXPOSE", "EXPOSE"], ["SCRIPT", "SCRIPT"], ["STANDARD", "STANDARD"]] }, 
    { "name": "ordering", "options": [["", "---------"], ["start", "Start"], ["-start", "Start (descending)"], ["end", "End"], ["-end", "End (descending)"], ["modified", "Modified"], ["-modified", "Modified (descending)"], ["created", "Created"], ["-created", "Created (descending)"], ["state", "State"], ["-state", "State (descending)"]] }
  ] 
};
...
describe('ObservationsTable.vue', () => {
  ...
  it('emits onSuccessfulDataRetrieval event on successful data retrieval', async () => {
    ...
    $.ajax
      .mockReturnValue(Deferred().resolve(filterOptions))
      .mockReturnValueOnce(Deferred().resolve(observationsList));
  ...
  it('emits onErrorRetrievingData event on successful data retrieval', async () => {
    ...
    $.ajax
      .mockReturnValue(Deferred().resolve(filterOptions))
      .mockReturnValueOnce(Deferred().reject(observationsList));
    const wrapper = wrapperFactory('http://localhost');
    await flushPromises();
  ...
  it('updates the route with API query params', async () => {
    ...
    const wrapper = wrapperFactory('http://localhost');
    await flushPromises();
    expect(wrapper.vm.$route.fullPath).toBe('/?state=COMPLETED&state=PENDING&state=IN_PROGRESS&state=ABORTED&state=FAILED&state=NOT_ATTEMPTED&ordering=-start&limit=50');
  });
